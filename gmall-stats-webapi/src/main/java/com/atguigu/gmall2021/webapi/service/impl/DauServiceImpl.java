...
public class DauServiceImpl implements DauService {
    ...
    JestClient jestClient;
     ...
     final  String INDEX_NAME_PREFIX="gmall0921_dau_info_";

     /*
       查询3件事  1 构造查询条件 2 提交到数据 3 提取查询结果
      */
    @Override
    ...
    public Map<String, Long> getDauHourCount(String date) {
        ...
        try {
            ...
            if(termsAggregation!=null){
                List<TermsAggregation.Entry> buckets = result.getAggregations().getTermsAggregation("groupby_hr").getBuckets();
                Map<String, Long> hourCountMap = new HashMap<>();
                for (TermsAggregation.Entry bucket : buckets) {
                    hourCountMap.put( bucket.getKey(),bucket.getCount());
                }
                return hourCountMap; //返回查询结果
            }else{
                return new HashMap<>();
            }
